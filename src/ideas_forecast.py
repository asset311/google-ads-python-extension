...
def create_ideas(client, customer_id, location_id, language_id, keyword, page_url, limit):
    ...
    keyword_plan_idea_service = client.get_service('KeywordPlanIdeaService', version='v2')
...
def plan_metrics(client, customer_id, location_id, frame):
    # create a keyword plan with plan id
    keyword_plan = create_keyword_plan(client, customer_id)
    logging.info('Forecasting period is {}'.format(F_interval))

    keyword_plan_campaign = create_keyword_plan_campaign(client, customer_id, location_id, keyword_plan[0])
    keyword_plan_ad_group = create_keyword_plan_ad_group(client, customer_id, keyword_plan_campaign)
    keyword_plan_keyword_service = client.get_service('KeywordPlanKeywordService', version='v2')
    ...
    for i in range(len(Ideas)):
        ind = create_keyword_plan_keywords(client, customer_id, keyword_plan_ad_group, Ideas[i], keyword_plan_keyword_service)
        indices.append(ind)

    frame = frame.join(pd.Series(np.array(indices), name = 'ind'))

    # generate forecast for each keyword(idea)
    resource_name = keyword_plan_service.keyword_plan_path(customer_id, keyword_plan[1])

    try:
        #generate forecast metrics for the overall keyword plan
    ...
    #make sure that all keywords have matches for 3 criteria (BROAD, EXACT, PHRASE)
    if len(response.keyword_forecasts) % 3 != 0:
        ...
        path = '/Users/Asset/iCloud/coding/zuri/google-ads-keyword-research/zuri-google-ads/data'
        output_file = os.path.join(path,title_res1)
        res1.to_csv(output_file, index = False)  #change to relative data folder
...
def create_keyword_plan_keywords(client, customer_id, plan_ad_group, idea, keyword_plan_keyword_service):
    ...
    keywords = [keyword_plan_keyword1, keyword_plan_keyword2, keyword_plan_keyword3]
    ...
    response = keyword_plan_keyword_service.mutate_keyword_plan_keywords(customer_id, operations)
    
    ind = response.results[0].resource_name.split('/')[-1]
    return ind

if __name__ == '__main__':
    google_ads_client = GoogleAdsClient.load_from_storage('/Users/Asset/iCloud/coding/zuri/google-ads-keyword-research/zuri-google-ads/google-ads.yaml')
